# Libro 1 — Derecho Aéreo y ATC · Texto de media-kit

---

## Blurb corto (redes sociales / catálogo)

> El cielo tiene reglas. El piloto que las conoce vuela con libertad. El que no, con miedo.

El **Libro 1 de Derecho Aéreo y ATC** del Manual SPL te explica el marco legal que rige cada vuelo: desde el Convenio de Chicago hasta las tres normas que debes conocer de memoria como piloto de planeador. Sin jerga jurídica innecesaria, con las consecuencias operativas de cada regla.

---

## Cita célebre

> «En aeronáutica, las reglas no se diseñan para limitar la libertad, sino para garantizar que todos volvamos a disfrutar de ella mañana.»
>
> — Anónimo, de la tradición oral de la seguridad aérea.

---

## Descripción larga (web / tienda / plataforma e-learning)

### Las reglas no son el enemigo. La ignorancia de las reglas, sí.

Hay pilotos que ven la normativa aeronáutica como un obstáculo burocrático. Pilotos con licencia que no saben con exactitud qué documentos deben llevar a bordo, qué espacio aéreo cruzan cada vez que salen del aeródromo, o qué hacer si reciben una intercepción de un avión militar. Esa ignorancia no es inocente: tiene consecuencias legales, operativas y, en los casos extremos, letales.

El **Libro 1: Derecho Aéreo y Procedimientos de Control de Tránsito Aéreo** está diseñado para que eso no te ocurra.

Con el mismo tono directo del instructor que ha volado durante décadas y ha llenado partes de incidente, este volumen cubre los catorce bloques temáticos del syllabus EASA AMC1 SFCL.130 sin convertirlos en un texto de oposiciones:

**Lo que encontrarás dentro:**

- **El marco internacional** — El Convenio de Chicago (1944) y la OACI que lo sustenta: por qué los 193 estados miembros de la OACI comparten unos estándares mínimos comunes y cómo eso te permite aterrizar con tus papeles en orden en cualquier aeródromo de Europa.

- **La arquitectura normativa EASA** — La diferencia entre un Reglamento de Ejecución (obligatorio) y un AMC (cómo cumplirlo). Por qué Part-SFCL regula tu licencia, Part-SAO tu operación, y SERA las reglas del aire. Y por qué confundir los tres niveles puede costarte la licencia.

- **Aeronavegabilidad y documentación** — Qué es el Certificado de Aeronavegabilidad, cuándo caduca el ARC y qué ocurre si despegas sin él. Los documentos que siempre deben acompañar a la aeronave.

- **Marcas, matrículas y registro** — Cómo se lee la matrícula de un planeador español, qué diferencia hay entre registro y certificado de aeronavegabilidad, y por qué la nacionalidad de la aeronave no siempre coincide con la del piloto.

- **Licencias: tus derechos y sus límites** — Los privilegios exactos de la SPL, las condiciones de experiencia reciente, las habilitaciones (TMG, acrobacia, remolque), y qué pasa si tu médico te da la baja.

- **Reglas del aire** — Las prioridades de paso que debes conocer de memoria (globos > planeadores > aviones de motor). Las reglas de prioridad en ladera y en el aterrizaje. La regla del antichoque a la derecha que ha salvado vidas.

- **El espacio aéreo en España** — Clases A hasta G, altitudes, condiciones de entrada y quién te dice si puedes cruzar. Las zonas restringidas, peligrosas y prohibidas que afectan al vuelo de vela.

- **Servicios de tránsito aéreo (ATS/ATM)** — La diferencia entre información de vuelo, alerta y control. Cuándo el ATC te da instrucciones obligatorias y cuándo solo te da información.

- **Información aeronáutica (AIS)** — La diferencia entre AIP (lo permanente), NOTAM (lo urgente) y AIC (lo informativo). El deber legal de consultarlos antes de cada vuelo, y cómo ENAIRE Insignia pone las restricciones sobre el mapa.

- **Procedimientos de navegación aérea** — Los mínimos VMC para volar visual y las excepciones que tenemos los planeadores. La regla semicircular para elegir altitud de crucero. La diferencia crítica entre QNH, QFE y QNE.

- **Aeródromos** — Categorías, indicadores de viento, señales en tierra y en pista. Lo que la «T» de aterrizaje del área de señales te está diciendo cuando te aproximas.

- **Seguridad (*security*)** — La diferencia entre evitar accidentes (*safety*) y prevenir actos ilícitos (*security*): cómo asegurar tu aeronave en tierra, comprobar que nadie la ha manipulado y qué artículos no pueden ir a bordo.

- **Búsqueda y salvamento (SAR)** — Las fases de incertidumbre (INCERFA), alerta (ALERFA) y socorro (DETRESFA). Qué activa cada una, quién las activa y qué debes hacer tú para no protagonizar innecesariamente un operativo SAR.

- **Notificación de accidentes e incidentes** — Cuándo tienes obligación de reportar, a quién, y por qué la cultura justa solo funciona si reportas incluso cuando el resultado fue favorable.

- **Derecho nacional** — La Ley de Seguridad Aérea, el reparto de papeles entre la DGAC (que define la política) y AESA (que vigila su cumplimiento), y el régimen sancionador: qué infracción es leve, cuál es muy grave y qué consecuencias tiene cada una.

---

### Para quién es este libro

Para el **alumno que prepara el examen teórico AESA SPL** y quiere entender el marco legal, no memorizar artículos que olvidará en tres semanas.

Para el **piloto licenciado** que lleva años volando sin haber repasado nunca Part-SFCL o que no recuerda exactamente qué espacio aéreo le rodea cuando sale a hacer circuitos.

Para **cualquier piloto VFR** que vuela en España y necesita saber con precisión qué puede hacer, dónde y con qué documentación.

---

### Por qué este manual es diferente

No es un listado de artículos de reglamento con definiciones subrayadas. Es una explicación de por qué existen esas normas y qué consecuencias operativas tiene cada una.

Cada capítulo traduce el lenguaje jurídico al lenguaje del hangar: no "SERA.3210 establece las prioridades de paso" sino "en convergencia, el avión de motor te cede el paso —pero eso no te autoriza a confiar en que el otro te ha visto".

Las cajas de **SEGURIDAD**, **NORMATIVA**, **REGLA DE ORO** y **AIRMANSHIP** separan la teoría de las decisiones concretas que tomas antes o durante el vuelo. El glosario al final recoge cada término con la definición precisa que necesitas para el examen.

**Syllabus:** EASA AMC1 SFCL.130 — Derecho Aéreo y ATC (AIR LAW)
**Formato:** PDF · EPUB · MD
**Licencia:** CC BY-SA 4.0

---

## Texto de contraportada

*(~120 palabras. Para imprimir en la contraportada del PDF encuadernado o en la página de presentación del EPUB.)*

---

**Tema 1 de 9 del examen teórico para la obtención de la licencia EASA de Piloto de Planeadores (SPL)**

Un planeador comparte el aire con tráfico que no improvisa, dentro de un marco de espacios y responsabilidades legales pensado para que ningún vuelo sea una sorpresa. Saltarse ese marco no te hace más libre; te deja más expuesto.

A lo largo de catorce capítulos el reglamento se vuelve algo que de verdad usas en cabina: qué documentos llevas encima, dónde necesitas autorización antes de entrar, qué dispara la fase de alerta SAR y por qué dar parte de un incidente es la única forma de que el sistema aprenda.

El cielo tiene reglas. Aprende a usarlas antes de despegar.

---

## Taglines alternativos

- *"No vuelas en el vacío: vuelas en un sistema de normas que protege tu vuelo y el de todos los demás."*
- *"De la OACI a tu cabina: el marco legal que regula cada lanzamiento, cada frecuencia y cada aterrizaje."*
- *"Part-SFCL, Part-SAO, SERA: las tres normas que un piloto de planeador debe conocer como la palma de su mano."*
- *"El derecho aéreo no es burocracia. Es el acuerdo colectivo que hace posible que dos planeadores no colisionen en la misma térmica."*

---

*Manual SPL — VuelaLibre.net · Basado en EASA AMC1 SFCL.130*
