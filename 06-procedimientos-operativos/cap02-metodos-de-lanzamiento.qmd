...
>
>
> * **El lanzamiento por torno**: dinámica, las cinco reglas de seguridad, fraseología y procedimiento de emergencia ante rotura de cable.
...
### Las cinco reglas del torno

Casi todo lo que sale mal en un torno sale mal **abajo**, en los primeros segundos y con el planeador aún rodando. A esa altura no hay tiempo de analizar nada, así que la escuela británica condensó la prevención en cinco reglas que se recitan antes de engancharse. Se enseñan en inglés y así viajan de club en club; van aquí con su original al lado.

::: {.callout-warning title="Seguridad: LAS CINCO REGLAS DEL TORNO"}
1. **Nunca lances con viento en cola.** *Never winch launch with a tail wind.*
2. **Nunca lances con la hierba alta.** *Never winch launch in long grass.*
3. **Prepárate para soltar ANTES de que el ala toque el suelo. LA MANO EN LA BOLA DE SUELTA.** *Be ready to release BEFORE the wing touches the ground. Hand on release.*
4. **Asegúrate de que quien lleva el ala está formado.** *Ensure wing runner is trained.*
5. **Cuidado con el viento cruzado.** *Be careful in crosswinds.*
:::

Las cinco tienen la misma raíz: **el planeador acelera de cero a volar en tres segundos, y durante ese rato el ala no tiene autoridad para corregir nada.** Vale la pena entender el porqué de cada una, porque una regla que sólo se memoriza se salta el día que estorba:

**Viento en cola.** El torno tira del cable, no del aire. Con viento en cola el planeador necesita mucha más carrera para alcanzar la misma velocidad indicada, el ángulo de trepada se desploma y la altura que ganas por metro de cable se hunde. Y si el cable rompe, la única pista que te queda por delante la tienes que aterrizar **también** en cola.

**Hierba alta.** La hierba frena la punta del ala que aún va por el suelo y agarra el cable. Con el planeador acelerando, esa punta retenida hace de pivote: el velero guiña, se pone de través y el ala de fuera se levanta. Es el arranque exacto de la regla 3.

**El ala que cae.** Si un ala toca el suelo con el planeador acelerando, el giro es violento e inmediato. La única acción útil es **cortar la tracción antes del contacto**, no después: se suelta al ver que el ala baja y no se corrige, no cuando ya raspa. Por eso la mano va en la anilla de suelta **desde que el cable se tensa**, no colocada «por si acaso» a mitad de carrera.

**Quien lleva el ala.** El *wing runner* —quien sujeta y corre con la punta— decide cómo empieza el lanzamiento. Sujetarla torcida, soltarla tarde, soltarla pronto o correr sin seguir el eje producen justo la caída de ala de la regla 3. No es un puesto de voluntario improvisado.

**Viento cruzado.** A baja velocidad el planeador quiere apuntar al viento y el ala de barlovento tiende a levantarse, con unos alerones que todavía casi no muerden. La técnica de mandos cruzados es la misma que en cualquier despegue y se detalla en el capítulo de operaciones especiales; lo propio del torno es que aquí dispones de **segundos**, no de metros de pista, para decidir si el lanzamiento sigue.

::: {.callout-tip title="Regla de oro"}
Las cinco se resuelven con la misma decisión, y es la más barata del día: **si algo no cuadra, no te enganches**. Un lanzamiento abortado en tierra cuesta cinco minutos. Los otros cuatro cuestan un planeador, y a veces más.
:::
...
1. **Gancho autorizado:** comprueba en el AFM y en las placas qué gancho corresponde a cada método. La disposición habitual es morro para remolque por avión y gancho próximo al CG para torno. El destinado al torno debe disponer de suelta automática (*back-release*).
...
* **Ganchos de remolque:** comprueba que los ganchos instalados estén limpios de tierra, óxido o grasa vieja. Acciona la anilla de suelta desde la cabina y verifica que el mecanismo abre por completo y vuelve a su posición cerrada sin quedarse a medias. La función de cada gancho es la indicada por el AFM y las placas del planeador.
...
::: {.callout-warning title="Seguridad: INCOMPATIBILIDAD DE ANILLAS"}
...
![Catálogo de fusibles de seguridad Tost: colores, resistencias nominales y referencias](imagenes/06-cap02-fusibles-tost.png){#fig-06-cap02-fusibles-tost width="70%"}
...
3. **Fallo a altura crítica y retorno:** definir la altura mínima para intentar un viraje de retorno seguro —como referencia, 150 metros AGL en lanzamiento por torno y 70 metros AGL en remolque por avión (ver @fig-06-cap07-emergencia-altura, sobre opciones de aterrizaje según altura y método de lanzamiento)— y decidir previamente hacia qué lado se virará, considerando la dirección y fuerza del viento actual para contrarrestar la deriva.
...
* **Las cinco reglas del torno**: ni viento en cola, ni hierba alta; **la mano en la bola de suelta** y soltar **antes** de que el ala toque el suelo; quien lleva el ala, formado; y cuidado con el viento cruzado. Las cinco protegen de lo mismo: los tres segundos en que el planeador acelera sin que el ala pueda corregir nada.
...
* **Comprobaciones previas**: gancho autorizado por el AFM, velocidades respetadas, mandos libres y comprobación cruzada. Revisa también las cuerdas (sin nudos), las anillas dobles Tost y el fusible de seguridad (**weak link**) que corresponda al planeador.
:::
