...
La herramienta más útil para combatir este sesgo cognitivo es fijar de antemano una **altura de decisión**. O mejor: una escalera de tres peldaños que convierte el descenso en un plan por fases, en lugar de un ultimátum:
...
Para evaluar si un campo es apto en los pocos segundos que tienes disponibles, utiliza la regla de las **7 S**. Este método te obliga a evaluar los factores correctos en el orden correcto, evitando que la urgencia te lleve a pasar por alto un riesgo crítico (@fig-06-cap05-7s-seleccion-campo):
...
::: {.callout-warning title="Seguridad: EL «CABALLITO» (GROUND LOOP)"}
Si durante el rodaje es inevitable el impacto a alta velocidad contra un obstáculo insalvable, puedes provocar un «caballito» deliberado (*ground loop*) llevando un ala al suelo y aplicando timón a fondo hacia ese mismo lado. El planeador pivotará violentamente sobre la punta del ala y se detendrá bruscamente, sacrificando la estructura del plano o del tren para disipar la energía y proteger a los ocupantes. Esta maniobra es un último recurso cuando la única alternativa es el choque frontal; recuerda que el material es reemplazable, pero la vida de los ocupantes no.
:::
...
* **Bloqueadores de mandos (gust locks):** asegura la palanca de mandos con el cinturón de seguridad y coloca bloqueadores externos en las superficies de control (timón, profundidad y alerones) si dispones de ellos, para evitar que el viento golpee las superficies contra sus topes físicos.
